...
export const recordKiss = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { count } = await supabase.from("kisses").select("id", { count: "exact", head: true }).eq("user_id", userId).gte("created_at", since);
...
export const myStats = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { supabase, userId } = context;
    const { data: all } = await supabase.from("kisses").select("id, city, party_id, partner_nickname, created_at, is_public").eq("user_id", userId).order("created_at", { ascending: false });
...
export const addFriend = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { username: string }) => z.object({ username: z.string().trim().toLowerCase().min(1).max(40) }).parse(d))
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    const { data: friend } = await supabase.from("profiles").select("id, username, display_name").eq("username", data.username).maybeSingle();
    if (!friend) throw new Error("Usuário não encontrado");
    if (friend.id === userId) throw new Error("Você não pode adicionar a si mesmo");
    const { error } = await supabase.from("friendships").insert({ user_id: userId, friend_id: friend.id });
...
export const listFriends = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: profiles } = await supabase.from("profiles").select("id, username, display_name, avatar_url, city").in("id", ids);
...
export const removeFriend = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    await supabase.from("friendships").delete().eq("user_id", userId).eq("friend_id", data.friendId);
    return { ok: true };
  });
